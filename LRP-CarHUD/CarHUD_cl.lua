...
local screenPosX = 0.190                  -- X coordinate (top left corner of HUD)
local screenPosY = 0.860                -- Y coordinate (top left corner of HUD)
...
local seatbeltColorOff = {180, 23, 41}      -- Color used when seatbelt is off
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            drawTxt(timeText, 4, locationColorText, 0.48, screenPosX, screenPosY +0.053)
			--PLACA DE VEHICULO -- PLATE VEHICLE
		 drawTxt(PlateVeh, 4, plateColorText, 0.47, screenPosX+0.053, screenPosY +0.054)
            
            -- Display heading, street name and zone when possible
            drawTxt(locationText, 4, locationColorText, 0.55, screenPosX, screenPosY + 0.088)
			...
			exports['mythic_notify']:DoCustomHudText('cajaazul', 'Acabas de Poner/Quitar el limitador de velocidad',6000)
                ...
                if ShouldUseMetricMeasurements() then
                    ...
                    drawTxt(("%.2d"):format(math.ceil(speed)), 2, speedColor, 0.60, screenPosX + 0.055, screenPosY + 0.016)
                    drawTxt("KPH", 2, speedColorText, 0.34, screenPosX + 0.077, screenPosY + 0.020) 
                else
                    -- Get vehicle speed in MPH and draw speedometer
                    local speed = currSpeed*2.23694
                    local speedColor = (speed >= speedLimit) and speedColorOver or speedColorUnder
                    drawTxt(("%.2d"):format(math.ceil(speed)), 2, speedColor, 0.50, screenPosX + 0.055, screenPosY + 0.041) --  screenPosX + 0.000, screenPosY + 0.025)
                    drawTxt("MPH", 2, speedColorText, 0.34, screenPosX + 0.077, screenPosY + 0.050) 
                end
                
                -- Draw fuel gauge
                local fuelColor = (currentFuel >= fuelWarnLimit) and fuelColorOver or fuelColorUnder
                drawTxt(("%.2d"):format(math.ceil(currentFuel)), 2, fuelColor, 0.60, screenPosX + 0.000, screenPosY + 0.017) --  screenPosX + 0.055, screenPosY + 0.025)
                drawTxt("GASO", 2, fuelColorText, 0.34, screenPosX + 0.022, screenPosY + 0.020) 

                --Draw cruise control status
				
                -- local cruiseColor = cruiseIsOn and cruiseColorOn or cruiseColorOff
                -- drawTxt("CRUSERO", 2, cruiseColor, 0.4, screenPosX + 0.040, screenPosY + 0.048)

                -- Draw seatbelt status if not a motorcyle
                if vehicleClass ~= 8 then
                    local seatbeltColor = seatbeltIsOn and seatbeltColorOn or seatbeltColorOff
                    drawTxt("CINTURON", 2, seatbeltColor, 0.50, screenPosX + 0.105, screenPosY + 0.052)
                   
                    drawTxt(" HP : "..vehenground , 4 , fuelColorText , 0.45, screenPosX + 0.105, screenPosY + 0.016 )
